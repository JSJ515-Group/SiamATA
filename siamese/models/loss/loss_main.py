...
from siamese.models.loss.GFocal_Loss import QualityFocalLoss, DistributionFocalLoss
import math
from siamese.core.config import cfg
from siamese.models.loss.iou_loss import IOULoss
...
class SiamFDBLossComputation(object):
    ...
    def __call__(self, locations, box_cls, box_regression, labels, reg_targets):
        ...
        if pos_inds.numel() > 0:
            ...
            if self.reg_max > 0:
                loss_dfl = self.dfl(pred_Integrate, self.bbox2distrance(reg_targets_flatten).reshape(-1),
                                    weight[:, None].expand(-1, 4).reshape(-1))
        else:
